...
namespace big
{
	    ...
	    m_dxgi_swapchain(*g_pointers->m_gta.m_swapchain)
	{
		...
		default_segu = io.Fonts->AddFontFromMemoryTTF(&seguoe, sizeof seguoe, 22, NULL, io.Fonts->GetGlyphRangesChineseFull());

		segu = io.Fonts->AddFontFromMemoryTTF(&seguoe, sizeof seguoe, 40, NULL, io.Fonts->GetGlyphRangesChineseFull());

		bold_segu = io.Fonts->AddFontFromMemoryTTF(&bold_segue, sizeof bold_segue, 40, NULL, io.Fonts->GetGlyphRangesChineseFull());

		ico = io.Fonts->AddFontFromMemoryTTF(&icon, sizeof icon, 24, NULL, io.Fonts->GetGlyphRangesChineseFull());

		ico_combo = io.Fonts->AddFontFromMemoryTTF(&icon, sizeof icon, 19, NULL, io.Fonts->GetGlyphRangesChineseFull());

		ico_button = io.Fonts->AddFontFromMemoryTTF(&icon, sizeof icon, 25, NULL, io.Fonts->GetGlyphRangesChineseFull());

		ico_grande = io.Fonts->AddFontFromMemoryTTF(&icon, sizeof icon, 100, NULL, io.Fonts->GetGlyphRangesChineseFull());
